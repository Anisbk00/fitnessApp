...
async function calculateDataCompleteness(userId: string): Promise<number> {
  ...
  if (user?.UserProfile) {
    factors++;
    if (user.UserProfile.heightCm) score += 0.2;
    if (user.UserProfile.birthDate) score += 0.2;
    if (user.UserProfile.biologicalSex) score += 0.2;
    if (user.UserProfile.primaryGoal) score += 0.2;
    if (user.UserProfile.activityLevel) score += 0.2;
  }

  // Check recent measurements
  if (user?.Measurement && user.Measurement.length > 0) {
    score += 0.3;
  }
