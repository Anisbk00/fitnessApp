...
export async function getUnsyncedWorkouts(): Promise<OfflineWorkout[]> {
  ...
  // Get all workouts and filter in JavaScript since IndexedDB doesn't support boolean keys
  const allWorkouts = await getOfflineWorkouts();
  return allWorkouts.filter(w => !w.synced);
}
...
export async function getUnsyncedFoodEntries(): Promise<OfflineFoodEntry[]> {
  ...
  // Get all food entries and filter in JavaScript since IndexedDB doesn't support boolean keys
  const allEntries = await getOfflineFoodEntries();
  return allEntries.filter(e => !e.synced);
}
