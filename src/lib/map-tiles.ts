...
// OpenStreetMap tile server (free, no API key required)
const TILE_SERVER = 'https://tile.openstreetmap.org';
// Alternative: 'https://a.basemaps.cartocdn.com/rastertiles/voyager'

// Cache settings
const MAX_CACHE_AGE = 30 * 24 * 60 * 60 * 1000; // 30 days
const MAX_TILES_IN_CACHE = 500;
...
export async function downloadTile(
  ...
  x: number,
  y: number,
  server: string = TILE_SERVER
): Promise<Blob> {
  const url = `${server}/${z}/${x}/${y}.png`;
  const response = await fetch(url);

  if (!response.ok) {
    throw new Error(`Failed to download tile ${z}/${x}/${y}: ${response.status}`);
  }
...
export function getTileUrl(z: number, x: number, y: number, server: string = TILE_SERVER): string {
