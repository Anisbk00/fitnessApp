...
import { useState, useEffect, useMemo, useReducedMotion, useCallback } from "react";
...
const ANALYTICS_CACHE_DB = 'analytics-cache';
const ANALYTICS_CACHE_STORE = 'data';
...
async function openAnalyticsCache(): Promise<IDBDatabase | null> {
  if (typeof window === 'undefined' || !window.indexedDB) return null;
  
  return new Promise((resolve) => {
    const request = indexedDB.open(ANALYTICS_CACHE_DB, 1);
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains(ANALYTICS_CACHE_STORE)) {
        db.createObjectStore(ANALYTICS_CACHE_STORE, { keyPath: 'key' });
      }
    };
  });
}
...
async function getCachedAnalytics(metric: string, range: string): Promise<AnalyticsData | null> {
  try {
    const db = await openAnalyticsCache();
    if (!db) return null;
    
    return new Promise((resolve) => {
      const tx = db.transaction(ANALYTICS_CACHE_STORE, 'readonly');
      const store = tx.objectStore(ANALYTICS_CACHE_STORE);
      const request = store.get(`${metric}-${range}`);
      
      request.onsuccess = () => {
        const cached = request.result as CachedAnalyticsData | undefined;
...
async function setCachedAnalytics(metric: string, range: string, data: AnalyticsData): Promise<void> {
  try {
    ...
    const tx = db.transaction(ANALYTICS_CACHE_STORE, 'readwrite');
    const store = tx.objectStore(ANALYTICS_CACHE_STORE);
    store.put(cached);
  } catch {
    // Silently fail
...
// ═══════════════════════════════════════════════════════════════
// EXPORT UTILITIES WITH PROVENANCE METADATA
// ═══════════════════════════════════════════════════════════════
...
function buildProvenanceMetadata(
  ...
  const dataPoints = data.graphData.length;
  const confidenceScore = data.nutrition?.metabolicStability || 0;
  
  const dataSources: string[] = [];
  if (data.bodyComposition?.currentWeight) dataSources.push('body_metrics');
  if (data.nutrition?.avgCalories > 0) dataSources.push('food_logs');
  if (data.training?.totalWorkouts > 0) dataSources.push('workouts');
  
  const processingNotes: string[] = [
    `Data aggregated over ${timeRange === '7d' ? '7 days' : timeRange === '30d' ? '30 days' : '90 days'}`,
    `Primary metric: ${metricMode}`,
    `Trend analysis: ${data.trend}`,
  ];
  
  if (dataPoints < 3) {
    processingNotes.push('Limited data points - trends may not be reliable');
  }
  
  return {
    exportedAt: new Date().toISOString(),
    exportedBy: 'Progress Companion Analytics',
    ...
    confidenceScore,
    processingNotes,
    checksum: generateChecksum(data.graphData),
  };
}

function exportToJSON(data: AnalyticsData, metricMode: string, timeRange: string): void {
  const provenance = buildProvenanceMetadata(data, metricMode, timeRange);
  ...
  a.href = url;
  a.download = `analytics-${metricMode}-${timeRange}-${new Date().toISOString().split('T')[0]}.json`;
...
function exportToCSV(data: AnalyticsData, metricMode: string, timeRange: string): void {
  const provenance = buildProvenanceMetadata(data, metricMode, timeRange);
  
  // Build CSV with provenance header
  const lines: string[] = [];
  
  // Add provenance as comments
  lines.push(`# Analytics Export - Progress Companion`);
  lines.push(`# Exported: ${provenance.exportedAt}`);
  lines.push(`# Time Range: ${provenance.timeRange}`);
  lines.push(`# Metric: ${provenance.metricMode}`);
  lines.push(`# Data Points: ${provenance.dataPoints}`);
  lines.push(`# Confidence: ${provenance.confidenceScore}%`);
  lines.push(`# Checksum: ${provenance.checksum}`);
  lines.push(`# Data Sources: ${provenance.dataSource.join(', ')}`);
  lines.push('#');
  lines.push('');
  
  // Add graph data
  lines.push('Date,Value,Trend');
  data.graphData.forEach((point, i) => {
    const prev = i > 0 ? data.graphData[i - 1]?.value : point.value;
    const trend = point.value > prev ? 'up' : point.value < prev ? 'down' : 'stable';
    lines.push(`${point.date},${point.value},${trend}`);
  });
  
  lines.push('');
  lines.push('# Body Composition');
  lines.push('Metric,Current,Previous,Change');
  lines.push(`Weight,${data.bodyComposition.currentWeight || ''},${data.bodyComposition.previousWeight || ''},${data.bodyComposition.weightChange || ''}`);
  lines.push(`Body Fat %,${data.bodyComposition.currentBodyFat || ''},${data.bodyComposition.previousBodyFat || ''},${data.bodyComposition.bodyFatChange || ''}`);
  lines.push(`Lean Mass,${data.bodyComposition.currentLeanMass || ''},${data.bodyComposition.previousLeanMass || ''},${data.bodyComposition.leanMassChange || ''}`);
  
  lines.push('');
  lines.push('# Nutrition Summary');
  lines.push('Metric,Value');
  lines.push(`Avg Calories,${data.nutrition.avgCalories}`);
  lines.push(`Avg Protein,${data.nutrition.avgProtein}`);
  lines.push(`Avg Carbs,${data.nutrition.avgCarbs}`);
  lines.push(`Avg Fat,${data.nutrition.avgFat}`);
  lines.push(`Metabolic Stability,${data.nutrition.metabolicStability}`);
  
  lines.push('');
  lines.push('# Training Summary');
  lines.push('Metric,Value');
  lines.push(`Total Workouts,${data.training.totalWorkouts}`);
  lines.push(`Total Volume,${data.training.totalVolume}`);
  lines.push(`Total Duration,${data.training.totalDuration}`);
  lines.push(`Recovery Score,${data.training.recoveryScore}`);
  
  const blob = new Blob([lines.join('\n')], { type: 'text/csv' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `analytics-${metricMode}-${timeRange}-${new Date().toISOString().split('T')[0]}.csv`;
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
...
export function AnalyticsPage() {
  ...
  const { dataVersion, isOnline } = useApp();
  
  // Respect user's reduced motion preference
  const prefersReducedMotion = useReducedMotion();
  
  // Fetch real analytics data with dataVersion watching
  const { data: fetchedData, isLoading, refetch } = useAnalytics(metricMode, timeRange, dataVersion);
  ...
  const handleExport = useCallback((format: 'json' | 'csv') => {
    ...
    if (format === 'json') {
      exportToJSON(data, metricMode, timeRange);
    } else {
      exportToCSV(data, metricMode, timeRange);
    }
    
    setShowExportMenu(false);
  }, [data, metricMode, timeRange]);
      ...
      >
        <div className="p-4 rounded-2xl bg-card/60 backdrop-blur-sm border border-border/30">
          ...
          <p className="text-xs text-muted-foreground mb-3">
            Export your analytics data with full provenance metadata including data sources, confidence scores, and processing notes.
          </p>
          ...
          <div className="flex gap-2">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-500/10 text-emerald-600 text-sm font-medium hover:bg-emerald-500/20 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
              aria-label="Export analytics data as JSON with provenance metadata"
            >
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-muted/50 text-muted-foreground text-sm font-medium hover:bg-muted transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
              aria-label="Export analytics data as CSV spreadsheet"
            >
              <Download className="w-4 h-4" aria-hidden="true" />
              CSV
            </motion.button>
          ...
          {data && (
            <div className="mt-3 pt-3 border-t border-border/30">
              <p className="text-[10px] text-muted-foreground">
                Exports include: timestamp, data sources ({analytics.bodyComposition?.currentWeight ? 'body_metrics, ' : ''}{analytics.nutrition?.avgCalories > 0 ? 'food_logs, ' : ''}{analytics.training?.totalWorkouts > 0 ? 'workouts' : ''}), confidence score, and checksum for data integrity verification.
              </p>
            </div>
          )}
        </div>
        ...
        {modes.map((mode) => {
          ...
          return (
            <motion.button
              ...
              aria-label={`View ${mode.label} analytics${isActive ? ' (currently selected)' : ''}`}
              className={cn(
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <p className="text-xs font-medium text-emerald-700 dark:text-emerald-400">AI Insight</p>
                <ProvenanceTag
                  source="model"
                  timestamp={new Date()}
                  rationale="Based on body composition data and progress trends"
                />
              </div>
