...
export function BarcodeScanner({
  ...
  const startScanner = useCallback(async () => {
    ...
    try {
      ...
      setHasPermission(false);
      setStatus("error");
      setStatusMessage("Camera access denied. Use manual input.");
    }
  }, [Html5Qrcode]);
  ...
  const onScanSuccess = async (decodedText: string) => {
    ...
    try {
      ...
      if (data.found && data.food) {
        ...
        setStatus("not_found");
        setStatusMessage("Product not found in database");
        onNotFound?.(decodedText);
      }
    } catch (error) {
      console.error("Barcode lookup error:", error);
      setStatus("error");
      setStatusMessage("Failed to lookup product");
    }
  };
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "bg-gradient-to-br from-emerald-500/20 to-teal-500/20 hover:from-emerald-500/30 hover:to-teal-500/30",
