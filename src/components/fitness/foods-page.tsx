...
import { BarcodeScanner, BarcodeScanButton, ScannedFood } from "./barcode-scanner";
...
interface Food {
  ...
  tags: string[];
  confidence: number;
  source?: 'global' | 'local' | 'manual'; // Where the food data comes from
}
...
function SearchFoodSheet({
  ...
  // Fetch foods from API when sheet opens or query changes
  useEffect(() => {
    if (!open) return;
    ...
    const fetchFoods = async () => {
      ...
      try {
        ...
        if (mealType === 'supplements') {
          params.set('supplementsOnly', 'true');
        } else {
        ...
        <div className="px-4 mb-4">
          ...
          <div className="mt-3">
            <button
              ...
              className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border border-emerald-500/20 flex items-center justify-center gap-2 text-emerald-600 dark:text-emerald-400 hover:from-emerald-500/20 hover:to-teal-500/20 transition-colors touch-manipulation"
            >
...
export function FoodsPage() {
  ...
  useEffect(() => {
    const fetchSupplementLogs = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/supplement-log?date=${date}`);
        if (response.ok) {
          const data = await response.json();
          const formattedLogs: MealEntry[] = (data.entries || []).map((entry: any) => ({
            id: entry.id,
            food: {
              id: entry.supplementId || entry.id,
              name: entry.supplementName || 'Unknown Supplement',
  ...
  const handleConfirmAdd = useCallback(async (food: Food, quantity: number, meal: MealType) => {
    ...
    const newCalories = (food.calories * quantity) / 100;
    const newProtein = (food.protein * quantity) / 100;
    const newCarbs = (food.carbs * quantity) / 100;
    const newFat = (food.fat * quantity) / 100;
    ...
    if (meal === 'supplements') {
      try {
        ...
        // Refresh supplement logs
        const date = new Date().toISOString().split('T')[0];
        const refreshResponse = await fetch(`/api/supplement-log?date=${date}`);
  ...
  }, [addFoodEntry, updateFoodEntry, editingEntry]);
  ...
  const handleBarcodeScan = useCallback((scannedFood: ScannedFood) => {
    // Convert ScannedFood to Food format
    const food: Food = {
      id: scannedFood.id,
      ...
      isVerified: scannedFood.isVerified,
      tags: [],
      confidence: scannedFood.source === 'local' ? 0.9 : 0.7,
      source: scannedFood.source === 'local' ? 'local' : 'global',
    };

    // Close scanner and open quick add dialog
