...
function ConfidenceBadge({ confidence, size = "default" }: { confidence: number; size?: "xs" | "sm" | "default" }) {
  const getColor = () => {
    if (confidence >= 80) return "bg-emerald-500/20 text-emerald-600 border-emerald-500/30";
    if (confidence >= 60) return "bg-amber-500/20 text-amber-600 border-amber-500/30";
  ...
  return (
    <Badge variant="outline" className={cn(getColor(), sizeClasses[size])}>
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="text-center">
        ...
        <div className="mt-3">
          <ConfidenceBadge confidence={scan.bodyFatConfidence} />
        </div>
      </div>
        ...
        <div className="p-4 rounded-xl bg-gradient-to-br from-violet-500/10 to-purple-500/10 border border-purple-500/20">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-foreground/80 mt-1 leading-relaxed">{scan.aiCommentary}</p>
            </div>
          </div>
        </div>
      )}
...
function MonthlySummaryCard({ summary }: { summary: NonNullable<ScanHistory["monthlySummary"]> }) {
  return (
    <Card className="overflow-hidden">
      <CardHeader className="pb-2 pt-4 px-4 bg-gradient-to-r from-violet-500/10 to-purple-500/10">
