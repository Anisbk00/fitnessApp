  ...
  return (
    <button
      ...
      className={cn(
        ...
        'hover:border-emerald-400/50 active:scale-[0.98]',
        selected
    ...
    >
      {suggested && confidence && (
        <div className="absolute -top-1 right-0.5 flex items-center gap-0.5 px-1 py-0.5 rounded-full bg-emerald-500/20 border border-emerald-500/30">
...
export function FinishSetupModal({
  ...
  const fetchSuggestions = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData(prev => ({
          ...prev,
          primaryGoal: data.suggestions.goalSuggestion.value as PrimaryGoal,
          activityLevel: data.suggestions.activitySuggestion.value as ActivityLevel,
          coachingTone: data.suggestions.toneSuggestion.value as CoachingTone,
        }));
      }
    } catch (err) {
  ...
  // Form validation
  const isValid = formData.primaryGoal && formData.activityLevel;
              ...
              {!isLoading && stage === 'form' && (
                <>
                  ...
                  <div className="flex flex-col items-center">
                    ...
                    >
                      <div className={cn(
                        'w-20 h-20 rounded-full bg-gradient-to-br from-emerald-400 to-teal-500',
